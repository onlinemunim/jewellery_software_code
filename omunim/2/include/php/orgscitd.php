...
$grvCity = $_POST['girviSearchCity'];

if ($grvCity == '') {
    $grvCity = $_GET['girviSearchCity'];
}
...
if ($_SESSION['sessionIgenType'] == $globalOwnPass) {
    ...
    $qSelTotalGirviCount = "SELECT girv_id FROM girvi where girv_own_id='$_SESSION[sessionOwnerId]' and girv_cust_city='$grvCity' and girv_firm_id IN ($strFrmId) and girv_upd_sts IN ('New','Updated','ReleaseCart') LIMIT $perOffset, $checkNextRows";
    $resTotalGirviCount = mysqli_query($conn,$qSelTotalGirviCount);
    $totalGirvi = mysqli_num_rows($resTotalGirviCount);

    $qSelAllGirvi = "SELECT * FROM girvi where girv_own_id='$_SESSION[sessionOwnerId]' and girv_cust_city='$grvCity' and girv_firm_id IN ($strFrmId) and girv_upd_sts IN ('New','Updated','ReleaseCart') order by STR_TO_DATE(girv_DOB,'%d %b %Y') desc, girv_pre_serial_num desc, girv_serial_num desc LIMIT $perOffset, $rowsPerPage";
    $resAllGirvi = mysqli_query($conn,$qSelAllGirvi);
    $totalNextGirvi = mysqli_num_rows($resAllGirvi);
} else if ($_SESSION['sessionIgenType'] == $globalOwnIPass) {

    $qSelTotalGirviCount = "SELECT girv_id FROM girvi where girv_own_id='$_SESSION[sessionOwnerId]' and girv_cust_city='$grvCity' and girv_upd_sts IN ('New','Updated','ReleaseCart') LIMIT $perOffset, $checkNextRows";
    $resTotalGirviCount = mysqli_query($conn,$qSelTotalGirviCount);
    $totalGirvi = mysqli_num_rows($resTotalGirviCount);

    $qSelAllGirvi = "SELECT * FROM girvi where girv_own_id='$_SESSION[sessionOwnerId]' and girv_cust_city='$grvCity' and girv_upd_sts IN ('New','Updated','ReleaseCart') order by STR_TO_DATE(girv_DOB,'%d %b %Y') desc, girv_pre_serial_num desc, girv_serial_num desc LIMIT $perOffset, $rowsPerPage";
    $resAllGirvi = mysqli_query($conn,$qSelAllGirvi);
    $totalNextGirvi = mysqli_num_rows($resAllGirvi);
}
?>
            ...
            <table border="0" cellspacing="1" cellpadding="0">
                <tr>
                    ...
                    <td valign="middle" align="left">
                        <div class="main_link_brown">Girvi List By City / Village:</div>
                    </td>
                    <td align="left">
                        <div class="main_link_green"><?php echo $grvCity; ?></div>
                    </td>
                    ...
                    <tr>
                        <td align="left">
                            ...
                            <input type="hidden" name="girviId<?php echo $gCounter; ?>" id="girviId<?php echo $gCounter; ?>" value="<?php echo $gId; ?>"/>
                             <div id="display_girvi_info_popup" class="girvi-info-popup_list"></div>
                        </td>
                        <td align="right">
