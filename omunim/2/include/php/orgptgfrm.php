...
if ($selMlName != '' && $selMlName != NULL && $selMlName != 'TR ML') {
    if ($selFirmId != NULL || $selFirmId != '') {
        $qSelAllGirvi = "SELECT * FROM girvi_transfer where gtrans_own_id='$_SESSION[sessionOwnerId]' and gtrans_upd_sts IN ('Transferred') and gtrans_exist_firm_id IN ($selFirmId) and gtrans_trans_loan_id IS NULL and gtrans_firm_id IN ($selTFirmId) and gtrans_act_sts IN ('UNSETTLED') order by STR_TO_DATE(gtrans_DOB,'%d %b %Y') desc, gtrans_serial_num desc";
    }
} else if (($selTFirmId == NULL || $selTFirmId == 'NotSelected' || $selTFirmId == '')) {
    $qSelAllGirvi = "SELECT * FROM girvi_transfer where gtrans_own_id='$_SESSION[sessionOwnerId]' and gtrans_upd_sts IN ('Transferred') and gtrans_exist_firm_id IN ($strFrmId) and gtrans_trans_loan_id IS NULL and gtrans_firm_id IN ($strGFirmId) and gtrans_act_sts IN ('UNSETTLED') order by STR_TO_DATE(gtrans_DOB,'%d %b %Y') desc, gtrans_serial_num desc";
} else {
...
<table width="100%">
 ...
 // ***************************************************************Start code change data from customer and supplier to user Table Author@: SANT12JAN16*******************************************************************                                      
 ...
 // ***************************************************************End code change data from customer and supplier to user Table Author@: SANT12JAN16*****************************************************    
...
<!--                                                <div id="transGirviDeleteButDiv">
                                                    <img src="<?php echo $documentRoot; ?>/images/delete16.png" width="13px" alt="" style="cursor: pointer;" 
                                                         onclick="javascript:unreleaseTransGirvi('<?php echo "$gId"; ?>', 'tFirm', '<?php echo $gTransGirviId; ?>', '<?php echo $prinType; ?>','<?php echo $upPanelName;?>');" />
                                                </div>-->
                                            </td>
                                        <?php } ?>
                                        </tr>
                        ...
                        <td align = "left" valign="top" width="50%" class="paddingLeft5">
                            ...
                            if ($selMlName != '' && $selMlName != NULL && $selMlName != 'TR ML') {
                                if ($selFirmId != NULL || $selFirmId != '') {
                                    $qSelAllRelGirvi = "SELECT * FROM girvi_transfer where gtrans_own_id='$_SESSION[sessionOwnerId]' and gtrans_upd_sts IN ('released') and gtrans_exist_firm_id IN ($selFirmId) and gtrans_trans_loan_id IS NULL and gtrans_firm_id IN ($selTFirmId) and gtrans_rel_sts IN ('UNSETTLED') order by STR_TO_DATE(gtrans_DOB,'%d %b %Y') desc, gtrans_serial_num desc";
                                }
                            } else if (($selTFirmId == NULL || $selTFirmId == 'NotSelected' || $selTFirmId == '')) {
                                $qSelAllRelGirvi = "SELECT * FROM girvi_transfer where gtrans_own_id='$_SESSION[sessionOwnerId]' and gtrans_upd_sts IN ('released') and gtrans_exist_firm_id IN ($strFrmId) and gtrans_trans_loan_id IS NULL and gtrans_firm_id IN ($strGFirmId) and gtrans_rel_sts IN ('UNSETTLED') order by STR_TO_DATE(gtrans_DOB,'%d %b %Y') desc, gtrans_serial_num desc";
                            } else {
                                $qSelAllRelGirvi = "SELECT * FROM girvi_transfer where gtrans_own_id='$_SESSION[sessionOwnerId]' and gtrans_upd_sts IN ('released') and gtrans_exist_firm_id IN ($strGFirmId) and gtrans_trans_loan_id IS NULL and gtrans_firm_id IN ($selTFirmId) and gtrans_rel_sts IN ('UNSETTLED') order by STR_TO_DATE(gtrans_DOB,'%d %b %Y') desc, gtrans_serial_num desc";
                            }
...
<!--                                            <td align="right" class="itemAddPnLabels12 border-bottom" width="22px">
                                    ...
                                    <td align="right" title="Total Interest Amount of This Page!" class="border-color-grey-rb" width="250px">
