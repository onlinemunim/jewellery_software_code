 ...
 * **************************************************************************************
 * 
 * Created on 12 OCT 2022 03:02 PM
 *
 * @FileName: omTallySellReturnVoucher.php
...
<ENVELOPE>
 ...
 <BODY>
  <IMPORTDATA>
   ...
   <REQUESTDATA>';
    $readWritefile = fopen("sellReturn.xml", "w");
    fwrite($readWritefile, "\n" . $sellVoucher);
    fclose($readWritefile);
    $qSelAllSellVoucher = "select * from user_transaction_invoice where utin_transaction_type='ItemReturn' and utin_firm_id='$firm_id' order by utin_invoice_no;";
//echo '$qSelAllSellVoucher'.$qSelAllSellVoucher.'<br>';
    ...
    while ($rowAllSellVoucher = mysqli_fetch_array($resAllSellVoucher, MYSQLI_ASSOC)) {
        ...
        $voucherid = $rowAllSellVoucher['utin_id']-2;
        $stockName = preg_replace('/[^A-Za-z0-9\-]/', ' ', $stockName);
        $sellVoucher = '<TALLYMESSAGE xmlns:UDF="TallyUDF">
     <LEDGER NAME="' . $user . ' S" RESERVEDNAME="' . $rowCustomerDetails['user_id'] . '" ACTION="Create">
      <MAILINGNAME.LIST TYPE="String">
       <MAILINGNAME>' . $user . ' S</MAILINGNAME>
      </MAILINGNAME.LIST>
      ...
      <LANGUAGENAME.LIST>
       ...
       <LANGUAGEID> 1033</LANGUAGEID>
          ...
          <VOUCHER REMOTEID="' . $voucherid . '"  SENDERID="eb86219e-6f73-4a24-8ffd-7d56994ae20a-00000007" VCHTYPE="Credit Note" ACTION="Create" OBJVIEW="Invoice Voucher View">
   ...
   <STATENAME>' . $custState . '</STATENAME>
      ...
      <PARTYNAME>' . $user . ' S</PARTYNAME>
      <PARTYLEDGERNAME>' . $user . ' S</PARTYLEDGERNAME>
      <REFERENCE>' .$voucherid. '</REFERENCE>
      <PARTYMAILINGNAME>' . $user . ' S</PARTYMAILINGNAME>
      ...
      <BASICBASEPARTYNAME>' . $user . ' S</BASICBASEPARTYNAME>
      <CSTFORMISSUETYPE/>
      <CSTFORMRECVTYPE/>
      ...
      <DUTYHEADDETAILS.LIST>      </DUTYHEADDETAILS.LIST>';
        $readWritefile = fopen("sellReturn.xml", "a");
        fwrite($readWritefile, "\n" . $sellVoucher);
        fclose($readWritefile);
        $qSelAllSellVoucher1 = "SELECT * FROM stock_transaction  where sttr_transaction_type='ItemReturn' and sttr_utin_id='$utin_id' and sttr_pre_invoice_no='$sttrPreInvoiceNo' and sttr_invoice_no='$st_invoice_no'";
