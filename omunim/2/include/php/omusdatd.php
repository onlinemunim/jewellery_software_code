...
//Udhaar Date
$dayDD = $_POST['dayDD'];
$monthMonth = $_POST['monthMonth'];
$yearYYYY = $_POST['yearYYYY'];

if ($dayDD == '') {
    $dayDD = $_GET['dayDD'];
    $monthMonth = $_GET['monthMonth'];
    $yearYYYY = $_GET['yearYYYY'];
}

$udhaarDate = $dayDD . ' ' . $monthMonth . ' ' . $yearYYYY;
...
$qSelTotalUdhaarCount = "SELECT utin_id FROM user_transaction_invoice where utin_owner_id='$_SESSION[sessionOwnerId]' and utin_date='$udhaarDate' and utin_type = 'udhaar' and utin_status IN ('New','Updated') LIMIT $perOffset, $checkNextRows";
$resTotalUdhaarCount = mysqli_query($conn,$qSelTotalUdhaarCount);
$totalUdhaar = mysqli_num_rows($resTotalUdhaarCount);
//
$qSelAllUdhaar = "SELECT * FROM user_transaction_invoice where utin_owner_id='$_SESSION[sessionOwnerId]' and utin_date='$udhaarDate' and utin_type = 'udhaar' and utin_status IN ('New','Updated') order by utin_since desc LIMIT $perOffset, $rowsPerPage";
                ...
                <tr>
                    <td>
                        <div class="spaceLeft20">
                            <h1>Udhaar List By Date:</h1>
                        </div>
                    </td>
                    <td align="left">
                        <div class="h1BlackColor"><?php echo $udhaarDate; ?></div>
                    </td>
                    ...
                    <table border="0" cellpadding="2" cellspacing="0" align="right">
                        <tr>
                            ...
                            if ($pageNum > 1) {
                                ?>
                                <td align="right">
                                    <form name="prev_udhaari" id="prev_udhaari"
                            ...
                            if ($totalUdhaar > $rowsPerPage) {
                                ?>
                                <td align="right">
                                    <form name="next_udhaari" id="next_udhaari"
                                          action="javascript:navigationUdhaarPanel(<?php echo $pageNum + 1; ?>);"
