...
/*
 * *************************************************************************************************
 * @tutorial: STOCK STATUS CHECK FILE FOR STOCK MANAGEMENT BY COUNTER @AUTHOR:PRIYANKA-25NOV2021
 * *************************************************************************************************
 * 
 * Created on 25 NOV, 2021 06:42:00 PM
 *
 * @FileName: omStockStatusCheckForStockManagementByCounter.php
        ...
        <td align="center">
            ...
            if ($newProductCode != '') {
                ...
                if ($newProductPostId != '') {
                    ...
                    //
                    //
                    // If Stock Available Count is Zero then check with Barcode 
                    ...
                    if ($stockAvailable == 0) {
                        ...
                        $querySelStockProduct = "SELECT sttr_id, sttr_sttrin_id, sttr_sell_status, "
                                              . "sttr_status, sttr_rfid_no, sttr_panel_name "
                                              . "FROM stock_transaction WHERE sttr_owner_id = '$_SESSION[sessionOwnerId]' "
                                              . "AND sttr_barcode = '$newProductCode' "
                                              . "AND sttr_transaction_type NOT IN ('sell', 'ESTIMATESELL', 'APPROVAL') ";
                        //
                        //echo '$querySelStockProduct == ' . $querySelStockProduct . '<br />';
                        //
                        ...
                        if ($stockAvailable > 0) {
                            $rowStockProduct = mysqli_fetch_array($resStockProduct, MYSQLI_ASSOC);
                            $productScanWithBarCode = $rowStockProduct['sttr_barcode'];
                            $productStatus = $rowStockProduct['sttr_status'];
                        }
                    }
                    //
                    //
                    //
                    // If Stock Available Count is Zero then check with Product Code 
                    ...
                    if ($stockAvailable == 0) {
                        ...
                        $querySelStockProduct = "SELECT sttr_id, sttr_sttrin_id, sttr_sell_status, "
                                              . "sttr_status, sttr_rfid_no, sttr_panel_name "
                                              . "FROM stock_transaction WHERE sttr_owner_id = '$_SESSION[sessionOwnerId]' "
                                              . "AND sttr_item_code = '$newProductCode' AND sttr_stock_type = 'retail' "
                                              . "AND sttr_transaction_type NOT IN ('sell', 'ESTIMATESELL', 'APPROVAL') ";
                        //
                        //echo '$querySelStockProduct == ' . $querySelStockProduct . '<br />';
                        //
                        ...
                        if ($stockAvailable > 0) {
                            $rowStockProduct = mysqli_fetch_array($resStockProduct, MYSQLI_ASSOC);
                            $productScanWithItemCode = $rowStockProduct['sttr_item_code'];
                            $productStatus = $rowStockProduct['sttr_status'];
                        }
                    }
                    //
                    ...
                    if ($stockPresent > 0) {
                        ...
                        $stockAvailable = 1;
                        //
                        // START CODE TO GETTING DETAILS OF WHOLESALE PRODUCT FROM STOCK TABLE IF PRODUCT POSTID IS EMPTY OR NULL @AUTHOR:PRIYANKA-25NOV2021 */
                        ...
                        //
                        // END CODE TO GETTING DETAILS OF WHOLESALE PRODUCT FROM STOCK TABLE IF PRODUCT POSTID IS EMPTY OR NULL @AUTHOR:PRIYANKA-25NOV2021 */
                        //
                    } else {
                        $stockAvailable = 1; ?>
                        <div class="fs_14 ff_calibri orange fw_b" style="margin-left: -50px; margin-top: 41px;">
